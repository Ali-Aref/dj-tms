...
Sample TMS backend for [TMSManager](../../TMSManager). It implements protocol v1 so a POS agent can register, heartbeat, report inventory, poll commands, and post results. There is no authentication. State lives in memory and is lost on restart.
...
| [Running](running.md) | Start the server, point the agent at it, read stdout logs |
...
| [Operator API](operator-api.md) | Health, inspect terminals, enqueue work by hand |
...
```text
POS agent                         Operator (curl / browser)
─────────                         ─────────────────────────
...
JSON request and response bodies are logged on every call. See [Running](running.md#logging).
