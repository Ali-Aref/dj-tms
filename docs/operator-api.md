...
Useful types today: `ping`, `collect_inventory`, `install_app`, `uninstall_app`, `reboot`. Payload rules: [Commands](commands.md). Invalid payload or missing terminal capability → `400` with an error string (e.g. `install_app: sha256 required`, `terminal does not support install_app`).
